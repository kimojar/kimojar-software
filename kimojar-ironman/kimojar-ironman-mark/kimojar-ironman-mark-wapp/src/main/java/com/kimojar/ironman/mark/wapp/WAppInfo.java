...
public class WAppInfo {
	...
	public static void deleteWAppCategory(String category) {
		List<String> toBeDeleteCategorys = new ArrayList<>();
		for(String cate : loadWAppInfo().keySet())
			if(cate.startsWith(category))
				toBeDeleteCategorys.add(cate);
		for(String delecate : toBeDeleteCategorys)
			loadWAppInfo().remove(delecate);
	}
