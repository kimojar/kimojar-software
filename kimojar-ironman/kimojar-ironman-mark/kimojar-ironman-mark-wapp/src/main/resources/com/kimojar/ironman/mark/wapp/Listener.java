...
public class Listener {
		...
		return new MouseAdapter() {
			...
			public void mouseExited(MouseEvent e) {
				...
				if(e.getComponent() instanceof JTextField) {
					...
					if(node.isLeaf() && node.getUserObject() instanceof WAppInfo) {
						WAppInfo app = (WAppInfo) node.getUserObject();
						JComboBox<?> combox = (JComboBox<?>) e.getComponent();
						if(combox == ComponentDict.lookup(WAppI18N.JComboBox_Category)) {
							String oldCategory = app.category;
							String newCategory = combox.getSelectedItem().toString().trim();
		...
		return new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(result == 0) {
					SwingUtilities.invokeLater(() -> {
						try {
							if(!appNode.isLeaf() || appNode.getUserObject() instanceof String) {
	...
	public static MouseListener generateCmdButtonClicked() {
		return new MouseAdapter() {
			...
			@Override
			public void mousePressed(MouseEvent e) {
				CommandGenerator.generateCommand(WAppInfo.loadWAppInfo());
				Object[] message = new Object[] {
				WAppI18N.Text_GenerateSuccess.i18n(),
				WAppI18N.Text_GenerateSuccessUsage.i18n(),
				};
		...
		return new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(categoryNode == null)
					return;
				if(!categoryNode.isLeaf() || categoryNode.getUserObject() instanceof String) {
